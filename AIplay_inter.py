# -*- coding: utf-8 -*-
"""
Created on Thu Dec 27 16:18:11 2018
...
def TestGetHF(AI,M):
    ...
    M.trick=5 #第6轮,已完成5轮
    for i in Positions:
        M.trick_history[i]=i+18
    M.current_player_position=2
    for i in range(M.trick):
        AI.TrickLeadingColors[i]=0 #之前的记录
        trick={0:0,1:1,2:2,3:3,'win':3}
        M.play_table.add(trick)
    for i in range(4):
...
def TGHFPrint(AI,bl=1):
    if bl:    
        ...
        print("AI.TrickNow ",AI.TrickNow)#trick
...
def AIPlaySet(AI):
    ...
    player = Dummy
    for c in range(52):
        print(AI.haveCardOut(c),end=' ')
    for c in range(N):
        print(AI.haveCard(AI.CardTable[c].Player,Card(c)),end=' ')
    print()
    for c in range(N):
        print(AI.haveCard(player,Card(c)),end=' ')
        
    for c in range(52):
        if AI.CardTable[c].Player is AI.PlayerNow:
            AI.CardsNow[c]=0
    print()
    print(AI.CardsNow)
